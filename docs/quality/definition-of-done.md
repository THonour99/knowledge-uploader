...
| 方 | 角色 | 性质 | 执行者 |
|---|---|---|---|
| 事实层 | 工具客观判定 | 可证伪、无主观 | `invoke check` / `invoke check-arm64`（或聚合入口 `invoke ship`） |
...
1. `invoke check` 全绿（聚合 `invoke lint` + `invoke test`）
2. `invoke check-arm64` 通过（无 ARM64 wheel 缺失、依赖锚定 `==`）
3. `invoke ship` 可作为发布/合并前事实层聚合入口（`check` + `check-arm64`）
