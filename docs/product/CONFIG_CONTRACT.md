...
> 当前契约恰好包含 25 个 active key 和 15 个 deleted key。
...
- ragflow.base_url 只能选择 `RAGFLOW_BASE_URL` 或环境所有者在
  `RAGFLOW_ALLOWED_BASE_URLS` 明确批准的完整 scheme/host/port/path；禁止 userinfo、query、
  fragment、实例 metadata 地址与跨端点重定向，数据库管理员不能扩大该边界。
...
1. 25 个 ConfigDefinition key 与 FALLBACKS 完全相等；
2. 25 个 key 与 FAIL_CLOSED_DEFAULTS 完全相等；
...
迁移 seed、管理 API 返回和前端设置页还必须通过行为测试证明是 25 active / 15 deleted。
