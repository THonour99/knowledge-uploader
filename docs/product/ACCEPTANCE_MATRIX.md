...
| ID | 验收场景 | 执行要点 | 预期 | 当前 |
|---|---|---|---|---|
| BASE-001 | 权威文档可追溯 | 检查 README/AGENTS/本地链接；穷举结构化与非结构化状态声明；校验 05 到 OpenAPI 的方法、路径和关键字段；以 exact clean HEAD 生成外部 JSON/日志 | 无断链；05 是唯一状态/API 契约；候选证据绑定 commit/tree 且不越界宣称外部发布 | 进行中（[本地基线验证器](./BASELINE_TRACEABILITY.md)已实现并通过开发期检查；最终整合后的 clean HEAD JSON/原始日志仍须由根 Agent 重跑归档） |
| AUTH-001 | 带部门注册 | 选择 active 部门注册，再查 `/auth/me` | 写正确 department id/name；role 仍 employee | 进行中（probe 已实现，本地聚合 3/3 与 auth 回归 50/50；最终候选 SHA 日志待归档） |
