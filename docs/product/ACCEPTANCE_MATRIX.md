...
> 版本：1.2 · 2026-07-18
>
> 状态值：`待执行`、`进行中`、`通过`、`失败`、`豁免`；尚未定版且会阻断发布的能力使用 `未完成（发布阻断）`。只有附 CI 链接/日志/截图/恢复记录的结果才能填“通过”；豁免必须有负责人、到期日和风险接受人。

## P0：基线与主链

| ID | 验收场景 | 执行要点 | 预期 | 当前 |
|---|---|---|---|---|
| BASE-001 | 权威文档可追溯 | 检查 README/AGENTS 引用与本目录链接；扫描结构化状态转换和阶段声明 | 无断链；05 是唯一状态/API 契约 | 进行中（[结构化基线检查](./BASELINE_TRACEABILITY.md)已通过 9 项，含可解析转换扫描；非结构化转换完整覆盖、API 方法/路径/字段语义与候选 commit 绑定日志待补） |
...
- P0 全部通过；`CFG/E2E/DLQ/OBS/ARM/DR`、`REL-001`、`EXT-SMTP-001`、`EXT-WEBHOOK-001`、`EXT-LLM-001`、`EXT-RAGFLOW-001` 均不允许豁免。
- `COST-002` 当前阻断任何会产生真实可计费 LLM 调用的候选发布；只有预算契约定版并附完整验收证据后才能解除，不能以实现方自行假设或人工口头承诺替代。
