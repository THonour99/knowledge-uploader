...
def _capture_rabbit_payload(
    ...
    class FakeProducer:
        def publish(self, body: object, **_kwargs: object) -> None:
            published.append(body)

    publisher = outbox_dispatcher.KombuEventPublisher("memory://")
    publisher._producer = cast(Producer, FakeProducer())
    monkeypatch.setattr(
        outbox_dispatcher,
        "dispatch_celery_task_for_event",
        lambda _event: 0,
    )
    publisher.publish(cast(EventOutbox, event))
    assert len(published) == 1
    assert isinstance(published[0], dict)
    return cast(dict[str, object], published[0])
...
async def test_register_writes_verification_outbox_without_replayable_token(
    ...
    assert set(outbox.payload) == {"user_id", "token_expires_at"}
    assert _capture_rabbit_payload(outbox, monkeypatch) == outbox.payload
    assert "verify@company.com" not in str(outbox.payload)
    assert "Verify" not in str(outbox.payload)
...
async def test_verify_email_activates_pending_user(
    ...
    async with AsyncSessionFactory() as session:
        outbox = (
            await session.execute(
                select(EventOutbox).where(EventOutbox.event_type == events.AUTH_USER_VERIFIED)
            )
        ).scalar_one()
    assert set(outbox.payload) == {"user_id", "verified_at"}
    assert _capture_rabbit_payload(outbox, monkeypatch) == outbox.payload
    assert "pending@company.com" not in str(outbox.payload)
...
async def test_forgot_password_writes_reset_outbox_without_replayable_token(
    ...
    assert set(outbox.payload) == {"user_id", "token_expires_at"}
    assert _capture_rabbit_payload(outbox, monkeypatch) == outbox.payload
    assert "forgot@company.com" not in str(outbox.payload)
    assert response.text.find(token.token_hash) == -1
...
async def test_resend_verification_response_hides_account_state_during_publisher_outage(
    ...
    async with AsyncSessionFactory() as session:
        outbox = (
            await session.execute(
                select(EventOutbox).where(
                    EventOutbox.event_type == events.AUTH_USER_VERIFICATION_RESENT
                )
            )
        ).scalar_one()
    assert set(outbox.payload) == {"user_id", "token_expires_at"}
    assert _capture_rabbit_payload(outbox, monkeypatch) == outbox.payload
    assert "pending-resend@company.com" not in str(outbox.payload)
