...
def ragflow_create_delete_task(file_id: str) -> str:
    return run_create_ragflow_delete_task(file_id)
...
@celery_app.task(name="ragflow.delete")  # type: ignore[misc]
def ragflow_delete_task(sync_task_id: str) -> str:
    return run_ragflow_delete_task(sync_task_id)
...
async def run_ragflow_delete_task_async(sync_task_id: str) -> None:
    task_id = uuid.UUID(sync_task_id)
    try:
        await _run_ragflow_delete_task(task_id)
    except Exception as exc:
        error_type = type(exc).__name__
        await _mark_ragflow_delete_task_failed(task_id, error_type)
        raise RuntimeError(error_type) from None
    finally:
        await engine.dispose()


async def _run_ragflow_delete_task(sync_task_id: uuid.UUID) -> None:
    async with AsyncSessionFactory() as session:
        service = RagflowTaskService(
            session=session,
            repository=RagflowTaskRepository(session),
...
async def _mark_ragflow_delete_task_failed(sync_task_id: uuid.UUID, error_type: str) -> None:
    async with AsyncSessionFactory() as session:
        service = RagflowTaskService(
            session=session,
            repository=RagflowTaskRepository(session),
