...
Knowledge Uploader — 公司内部知识库文件贡献与 RAGFlow 同步平台。员工通过 Web 上传文档，平台完成文件校验、去重、AI 分析、管理员审核、RAGFlow Dataset 同步，最终提供给下游问答服务。
...
所有业务响应统一 envelope：`{ success, data, message, request_id, error_code? }`。认证使用 Bearer JWT。三种角色：`employee`、`dept_admin`（部门管理员）、`system_admin`。
