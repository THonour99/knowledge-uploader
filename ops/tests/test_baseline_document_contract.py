...
def test_base_001_records_progress_without_self_certifying_completion() -> None:
    ...
    assert len(rows) == 1
    assert "进行中（[结构化基线检查](./BASELINE_TRACEABILITY.md)已通过" in rows[0]
    ...
    assert "不是候选发布证据" in evidence
    assert "不能独立支撑 `BASE-001` 标记“通过”" in evidence
    assert "不替代运行时、真实基础设施、外部服务、ARM64 或发布证据" in evidence
